...
const GameData = () => {
  ...
  const fetchGameData = async () => {
    try {
      const response = await axios.get(`http://localhost:5001/game-data/${walletAddress}`);
  ...
  return (
    ...
    <h1 className="text-2xl font-bold mb-4 w-full text-center">Game Data for Wallet: {walletAddress}</h1>
          ...
          <tbody>
            {gameData.map((data, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-100"}>
                ...
                <td className="border border-gray-300 p-2">{data.player_score}</td>
                <td className="border border-gray-300 p-2">{data.computer_score}</td>
                <td className="border border-gray-300 p-2">{new Date(data.timestamp).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
